...
const command: SlashCommand = {
    ...
    execute: async interaction => {
        ...
        if (!user) {
            if (targetUser) return interaction.reply("This person hasn't even made a bank account in 'Yun Banks™' yet, trying to access their balance you're so silly billy");
            message = "You have not made a bank account in 'Yun Banks™' yet, I will make one for you <3"
            createUser(userID);
        }
