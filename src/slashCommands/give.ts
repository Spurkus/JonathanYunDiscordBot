...
import { getUser, createUser, addToWallet, removeFromWallet } from "../economy";
...
const command: SlashCommand = {
    ...
    execute: async interaction => {
        ...
        if (!targetExists) return interaction.reply("You gotta tell me who you're giving YunBucks to :sob:");
        if (!amount) return interaction.reply("You have to specify an amount you're giving to someone");
        ...
        if (!user) {
            createUser(userID);
            return interaction.reply("You have not made a bank account in 'Yun Banks™' yet, and you're already trying to give people YunBucks :sob:.\nIt's ok, I will make one for you <3");
        }
        ...
        if (user.wallet < amount) return interaction.reply("You can't have enough YunBucks in your wallet to give to the person :3");
